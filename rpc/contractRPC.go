...
import (
	...
	berry1 "github.com/berry-data/BerryMiner/contracts1"
...
type contractWrapper struct {
	contract    *berry1.BerryTransactor
	contract2   *contracts.BerryMaster
...
func PrepareContractTxn(ctx context.Context,proxy db.DataServerProxy, ctxName string, callback berryCommon.TransactionGeneratorFN) error {
	...
	client := ctx.Value(berryCommon.ClientContextKey).(ETHClient)
	...
	for i < 5 {
		...
		instance := ctx.Value(berryCommon.TransactorContractContextKey).(*berry1.BerryTransactor)
